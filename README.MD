...
print("Backup Time (s)", full_backup_time)      # 0.10952234268188477
print("Backup Size (bytes)", full_backup_size)  # 43855872
```

### Incremental backup

```python
def incremental_backup_v2(src_db, backup_path, last_backup_path):
...
start_time = time.time()
incremental_backup_v2(db_name, incremental_backup_path, full_backup_path)
...
print("Backup Time (s)", incremental_backup_time)     # 0.01912069320678711
print("Backup Size (bytes)", incremental_backup_size) # 16384
```
...
print("Backup Time (s)", reverse_delta_backup_time)     # 1.905756950378418
print("Backup Size (bytes)", reverse_delta_backup_size) # 12288
```

## Compare their parameters: size, ability to roll back at specific time point, speed of roll back, cost.

| Backup Type   | Size (bytes) | Rollback Ability       | Rollback Speed          | Cost                                     |
|---------------|--------------|------------------------|-------------------------|------------------------------------------|
| Full          | 43,855,872   | One specific point     | Fast                    | High storage, high cost                  |
| Incremental   | 16,384       | Multiple points        | Slower                  | Low storage, low cost                    |
| Differential  | 16,384       | Since last full backup | Faster than Incremental | Moderate storage & cost                  |
| Reverse Delta | 12,288       | Multiple points        | Variable speed          | Low storage, moderate computational cost |
